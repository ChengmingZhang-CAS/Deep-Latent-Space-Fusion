"""
Deep latent space fusion for multi-omics data 
...
def affinity_fusion(omics_list, num_features=2000, type='min-max', neighbor_num='default', data_name="fusion",
                    epochs=1000, lr=1e-3, weight_xx=1.0,
                    weight_zz=0.2, weight_selfExp=1.0, weight_coef=1.0,
                    omics_weights=None, device='cpu', show_freq=10, plot_loss=False):
    ...
    if plot_loss:
        ...
        # plt.show()
        plt.savefig('{} omics training loss.png'.format(param_name))
...
if __name__ == "__main__":
    ...
    weight_coef = 0.10  # coefficient regularization
    w = [0.5, 0.5, 0.5]
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    show_freq = -1  # "-1" means we don't print the loss
    # ========================== Deep multi-omics subspace clustering ==========================
    param_name = "{}-{}-{}".format(str(weight_zz), str(weight_selfExp), str(weight_coef))

    # weight_selfExp = round(weight_selfExp, 2)
    _, C = affinity_fusion(omics_list, num_features=2000, type='min-max', neighbor_num='default',
                           data_name=data_name, epochs=epochs, lr=1e-3,
                           weight_xx=1.0, weight_zz=weight_zz, weight_selfExp=weight_selfExp,
                           weight_coef=weight_coef, omics_weights=w, device=device, show_freq=show_freq,
                           plot_loss=False)
    # plt.figure()
    # sns.heatmap(C)
    ...
    fused = (C + C.T) / 2
    pd.DataFrame(fused).to_csv(ret_file + '\\' + param_name + '-sym-C.csv')
    # plt.figure()
    sns.heatmap(fused)
    plt.savefig('{}.png'.format(ret_file + '\\' + param_name + '-sym-C'))
    plt.close()
    # ========================== Spectral clustering ==========================
    ...
    # you can use any clustering method here, but since DMOSC returns an affinity
    # matrix (i.e., all entries are positively-valued and indicate similarity)
    # spectral clustering makes a lot of sense
    labels_pred = cluster.spectral_clustering(fused, n_clusters=cluster_num) + 1
    fused_cluster = pd.DataFrame(labels_pred, index=samples_id, columns=['cluster'])
    end = time.time()
    print("Time: ", end - start)
    fused_cluster.to_csv(ret_file + '\\' + param_name + "_fused_cluster_max.csv")

    # # load the label data y
    # raw_df = pd.read_csv("fused_cluster_second.csv", header=0, index_col=0)
    # labels_true = raw_df['cluster']
    #
    # print("label raw:", Counter(labels_true))
    ...
    # # ARI_df.loc[k, weight_coef] = coef_ARI
    # # ARI_df.to_csv(ret_file + '\\' + "knn_ARI_0.0-0.2.csv")
    print("Finished!!!")
