...
SIMULATION.DATA = list(
  list(name='CLL', only.primary=T, is.rna.seq=T, is.mirna.seq=T, display.name='CLL'))



MAX.NUM.CLUSTERS = 15

OMIC.SUBSETS = list('multi_omics', 'exp', 'methy', 'mutaion')
...
set.omics.list.attr <- function(subtype.raw.data, subtype.data) {
  ...
  attr(subtype.raw.data[[3]], 'is.seq') = F
...
write.clusterings.to.csv <- function(benchmark.ret) {
  ...
  for (i in 1:length(SIMULATION.DATA)) {
    ...
    for (omic in c('all', '1', '2', '3')) {
      ...
      for (algorithm.name in ALGORITHM.NAMES) {
        sample_id = names(all.clusterings[[subtype]][[algorithm.name]][[omic]])
      ...
      rownames(cur.clusterings) = sample_id
...
get.raw.data <- function(subtype, only.primary=NA) {
  ...
  omics.files = c("aligned_mRNA.csv", "aligned_Methylation.csv", "aligned_Mutations.csv")
  omics.dir = paste0(subtype, "_", omics.files)
  raw.data = list()
  for (i in 1:length(omics.files)){
    print(paste("omics: ", i))
    raw.data[[i]] = read.csv(omics.files[i], header = TRUE, row.names = 1)
  }
